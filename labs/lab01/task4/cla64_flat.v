...
//       (p[18] & p[17] & p[16] & p[15] & p[14] & p[13] & p[12] & p[11] & p[10] & p[9] & p[8] & p[7] & p[6] & p[5] & g[2]) |
...
//       (p[22] & p[21] & p[20] & p[19] & p[18] & p[17] & p[16] & p[15] & p[14] & p[13] & p[12] & p[11] & p[10] & p[9] & p[7] & g[6]) |
...
//   // c[26] through c[64] follow the exact same expanded pattern.
...
module cla64_flat(
  ...
  // c[11] through c[64] are generated using the same direct CLA pattern.
  // To avoid transcription errors, use the following recursive expansion
  // generated by the exact CLA equation pattern.
  ...
  /*
   * The remaining carries are most safely generated from the same
   * recurrence pattern:
   *
   * c[k] = g[k-1]
   *      | p[k-1]g[k-2]
   ...
   *      | p[k-1]...p[0]cin
   *
   * For a compilable flat implementation without manually creating
   * hundreds of intermediate gates, the equivalent direct carry
   * recurrence below is used.
   */
